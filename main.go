...
func main() {
	...
	p.RegisterProtocol(protocol.NewCyclonProtocol(p, conf))
	p.StartSync()
}

func readConfFile(path string) *protocol.CyclonConfig {
	f, err := os.Open(path)
	if err != nil {
		panic(err)
	}
...
func ParseBootstrapArg(arg *string, conf *protocol.CyclonConfig) {
	if arg != nil && *arg != "" {
